...
# Declare varaible 
testX = pd.read_csv('Processed Data/testX_20190529.csv', header=0,low_memory=False)
testY = pd.read_csv('Processed Data/testY_20190529.csv', header=0,low_memory=False)
trainX = pd.read_csv('Processed Data/trainX_20190529.csv', header=0,low_memory=False)
trainY = pd.read_csv('Processed Data/trainY_20190529.csv', header=0,low_memory=False)
...
def train(train_test_col):
    ...
    overall["pred_plc"] = overall.groupby(['date', 'raceNo'])["pred_finishTime"].rank()
    overall["real_plc"] = overall.groupby(['date', 'raceNo'])["plc"].rank()

    overall = overall[(overall['pred_plc'] <= 1) & (overall['odds'].astype(float) <= 15) & (overall['odds'].astype(float) >= 5)]
    overall.loc[overall['real_plc'] <=3, 'real_first_3'] = 1
    overall.fillna(0,inplace=True)
    first_1 = accuracy_score(overall['real_plc'].round(0), overall['pred_plc'].round(0))
    ...
    with first_1_max.get_lock(),first_3_max.get_lock():
        ...
        if (first_1 + first_3) > (first_1_max.value + first_3_max.value):
            print('updated')
            logging.info('Accuracy (All) first_1: %.4f, first_3: %.4f, col: %s', first_1, first_3,train_test_col)
            first_1_max.value = first_1
            first_3_max.value = first_3
        else:
            print('perious:',first_1_max.value , first_3_max.value, 'current: ',first_1, first_3 )
